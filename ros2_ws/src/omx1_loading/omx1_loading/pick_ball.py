...
def validate_motion_target(
    ...
    *,
    ...
    """실행할 XYZ 한 점이 관절 한계(와 선택적으로 작업 영역)를 만족하는지 확인한다.

    ``SAFE_X_RANGE``/``SAFE_Y_RANGE``는 공을 **집는** 영역이라 바구니(놓는
    위치)는 보통 그 밖에 있다. 배치 목표는 ``require_workspace=False``로
    역기구학 도달 여부만 확인한다.
    """
    if require_workspace and not is_in_safe_rectangle(x, y):
        raise ValueError(
...
def validate_pick_place_plan(
    ...
    검증하는 높이는 ``_execute_action``/``loading_node``가 실제로 명령하는
    값(``pick_place_tuning``의 여유를 더한 값)과 같아야 한다 — 캘리브레이션
    원값으로 검사하면 통과해도 실제 명령은 실패할 수 있다.
    """
    rx, ry, _ = robot_xyz
    px, py = calibration.place_pos
    travel_z = calibration.approach_z + APPROACH_Z_MARGIN
    pick_targets = (
        ("물체 접근", rx, ry, travel_z),
        ("물체 집기", rx, ry, calibration.pick_z + PICK_Z_MARGIN),
    )
    place_targets = (
        ("배치 접근", px, py, travel_z),
        ("배치", px, py, calibration.place_z + PLACE_Z_MARGIN),
    )
    for label, x, y, z in pick_targets:
        validate_motion_target(label, x, y, z)
    for label, x, y, z in place_targets:
        validate_motion_target(label, x, y, z, require_workspace=False)
...
def validate_calibration_workspace(calibration: OmxCalibration) -> None:
    ...
    대응점에서 걸리는 게 있어도 실행을 막지는 않는다 — 실제 pick 루프의
    ...
    반면 배치점(``place_pos``)은 모든 pick이 공통으로 거치는 곳이라 닿지
    않으면 매번 공을 집은 뒤에 실패한다. 배치 접근·놓기 높이는 여기서
    예외로 막는다.
    """
    outside_rectangle = [
    ...
    px, py = calibration.place_pos
    travel_z = calibration.approach_z + APPROACH_Z_MARGIN
    validate_motion_target("배치 접근", px, py, travel_z, require_workspace=False)
    validate_motion_target(
        "배치",
        px,
        py,
        calibration.place_z + PLACE_Z_MARGIN,
