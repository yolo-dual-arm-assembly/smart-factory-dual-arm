...
from omx1_loading.pick_ball import is_safe_approach_target, validate_pick_place_plan
...
class ArmControllerNode(Node):
    ...
    def command_move(self, msg: DetectionResult) -> None:
        ...
        if not is_safe_approach_target(*approach_target):
            ...
            self.hit_count = 0
            return
        # 집은 뒤에 배치점이 안 닿는 걸 알면 늦다 — 전체 경로를 첫 모터 명령
        # 전에 확인한다 (pick_ball.OmxVisionRunner와 같은 정책).
        try:
            validate_pick_place_plan((rx, ry, cal.pick_z), cal)
        except ValueError as error:
            self.get_logger().error(f"Pick & Place 계획 거부 — {error}")
            self.tracking = False
            self.hit_count = 0
            return
